...
class GameScene: SKScene, SKPhysicsContactDelegate{
    ...
    override func didMove(to view: SKView) {
        ...
        canRestart = true
        verticalPipeGap -= Double(50*levelDifficulty)
        // setup physics
        ...
        let movePipes = SKAction.moveBy(x: -distanceToMove, y:0.0, duration:TimeInterval(CGFloat(levelSpeed) * distanceToMove))
        ...
        bird.setScale(2.0)
        bird.position = CGPoint(x: self.frame.size.width * 0.35, y:self.frame.size.height * 0.8)
        bird.run(flap)
    ...
    func gameOver(){
        gameDelegate?.setPlayerScore(valueSent: Int(score))
        gameDelegate?.toHighscores()
    }
}
